...
|--------|-----|-------|---------|
| **VPS1** | **Домашний** (внутри РФ) | «Входной» — к нему подключается ваш телефон/ноутбук. На нём же живёт веб-панель управления. | 1 CPU, 1 GB RAM, Ubuntu 24.04 или Debian 12 |
| **VPS2** | **Зарубежный** (любая страна вне РФ) | «Выходной» — через него вы выходите в интернет | 1 CPU, 1 GB RAM, Ubuntu 24.04 или Debian 12 |
...
Нужен только для одного: **одного раза подключиться к VPS1 по SSH** и вставить команду. После этого всё делается через браузер.

- Любой терминал с SSH (Mac: `Terminal.app`, Windows: встроенный PowerShell / [PuTTY](https://www.putty.org/), Linux: любой).
- Браузер (Chrome, Firefox, Safari, Edge).

Никаких `git`, `ansible`, `pipx`, `Docker` на вашем компьютере не нужно.

### 3. Клиент (приложение на телефоне/ноутбуке)
...
Если с командной строки (на вашем компьютере или VPS1):
